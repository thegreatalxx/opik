...
export const EXPLAINERS_MAP: Record<EXPLAINER_ID, Explainer> = {
  ...
  [EXPLAINER_ID.why_do_i_need_multiple_test_suites]: {
    ...
    description:
      "Create datasets and test suites to evaluate your LLM application in different contexts, scenarios, or environments.",
  },
  ...
  [EXPLAINER_ID.what_does_the_test_suite_do_here]: {
    ...
    description:
      "Run a prompt on your test suite to preview its performance. You can select metrics, filter the data, or adjust the page size. The experiment runs only on the items currently shown.",
  },
  [EXPLAINER_ID.how_do_i_use_the_test_suite_in_the_playground]: {
    id: EXPLAINER_ID.how_do_i_use_the_test_suite_in_the_playground,
    description:
      "Use mustache syntax to reference test suite variables in your prompt. Example: ",
  },
  ...
  [EXPLAINER_ID.metric_reference_key]: {
    ...
    description:
      "The key in the test suite item to compare against. Supports JSONPath for nested values (e.g., $.scores[?(@.name=='Useful')].value).",
  },
