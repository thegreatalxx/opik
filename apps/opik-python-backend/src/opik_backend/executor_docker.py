...
payload_code_size_histogram = meter.create_histogram(
    name="payload_code_size",
    description="Size of the code payload in bytes",
    unit="bytes",
)

payload_data_size_histogram = meter.create_histogram(
    name="payload_data_size",
    description="Size of the data payload in bytes",
    unit="bytes",
)

# Counter for execution outcomes to track failure rates
execution_outcome_counter = meter.create_counter(
    name="execution_outcome",
    description="Count of execution outcomes by status (success, timeout, invalid_code, error)",
    unit="1",
)
...
class DockerExecutor(CodeExecutorBase):
    ...
    def _start_pool_monitor(self):
        ...
        logger.debug(f"Starting container pool monitor with {self.pool_check_interval} second interval")
    ...
    def _check_pool(self):
        ...
        if self.stop_event.is_set():
            logger.debug("Container pool monitor stopped")
    ...
    def _run_scheduler(self):
        ...
        logger.debug("Starting scheduler for container pool monitoring")
        while not self.stop_event.is_set():
            schedule.run_pending()
            time.sleep(1)  # Sleep to avoid busy-waiting

        logger.debug("Scheduler finished running")
    ...
    def _pre_warm_container_pool(self):
        ...
        """
        logger.debug(f"Pre-warming container pool with {self.max_parallel} containers")
    ...
    def ensure_pool_filled(self):
        ...
        with self.pool_lock:
            while not self.stop_event.is_set() and len(self.get_managed_containers()) < self.max_parallel:
                logger.debug("Not enough python runner containers running; creating more...")
    ...
    def _record_execution_outcome(self, status: str, payload_type: Optional[str] = None):
        ...
        execution_outcome_counter.add(1, attributes={
            "status": status,
