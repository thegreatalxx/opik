...
<Note>
  In Opik 2.0, Experiments and Test Suites are project-scoped. Make sure to specify a `project_name` when creating datasets and running experiments.
</Note>
...
dataset = opik_client.get_or_create_dataset("Evaluation test dataset", project_name="my-project")
dataset.insert([
...
opik_client = opik.Opik()
dataset = opik_client.get_or_create_dataset("RAG evaluation dataset", project_name="my-project")
dataset.insert([
    {
