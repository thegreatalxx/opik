---
description:
  Start here to integrate Opik into your OpenAI-based genai application
  for end-to-end LLM observability, unit testing, and optimization.
headline: OpenAI | Opik Documentation
og:description:
  Monitor and debug OpenAI API calls effortlessly using Opik's tracing
  features and the official OpenAI Node.js SDK.
...
<Info>
  Looking for the OpenAI Node.js or JavaScript integration? This page covers the
  official `openai` SDK together with `opik` and `opik-openai` for TypeScript
  and Node.js applications.
</Info>

## Quickstart

The fastest way to set up Opik in an existing OpenAI Node.js project is:

```bash
npx opik-ts configure
```

On Opik Cloud, the wizard validates your API key, suggests your default
workspace automatically, and writes the required Opik environment variables for
you.
...
These packages are used together:

- `openai` for the official OpenAI Node.js SDK
- `opik` for the core Opik client and configuration
- `opik-openai` for the `trackOpenAI` wrapper
...
If you are configuring Opik Cloud manually, set the following environment variables:
...
<Tip>
  `OPIK_WORKSPACE` is required for Opik Cloud. If you use `npx opik-ts
  configure`, the CLI can validate your API key and prefill your default
  workspace for you.
</Tip>
...
**Module Resolution Errors**: Install all three packages together: `opik`, `openai`, and `opik-openai`.
