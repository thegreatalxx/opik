...
Create and automatically sync a text prompt using direct instantiation:

```typescript
import { Prompt, PromptType } from "opik";
...
Get persisted prompts from the backend by name or specific version:
...
Prompts automatically create new versions when content changes:
...
// Same template, metadata, and type - reuses version 1
...
// Verify they have the same commit
console.log(prompt2.commit === prompt1.commit); // true

// Changed template - creates version 2
...
// Changed metadata - creates version 3
...
// Later, search by tags using client
import { Opik } from "opik";
const client = new Opik({ projectName: "my-project" });
const prodPrompts = await client.searchPrompts('tags contains "production"');
```
...
Create and automatically sync a chat prompt using direct instantiation:
...
// Retrieve a persisted chat prompt
...
Get persisted chat prompts from the backend by name or specific version:
