...
og:description: Understand the two evaluation approaches in Opik — Evaluation Suites
...
## Evaluation Suites — assertion-based testing

Evaluation Suites let you define expected behaviors as natural-language assertions. An LLM judge checks each assertion against your agent's output and reports pass/fail results.
...
| | Evaluation Suites | Datasets & Metrics |
|---|---|---|
...
You can use both approaches together. For example, use Evaluation Suites during development to validate specific behaviors, and Datasets & Metrics in CI to track quality scores over time.
