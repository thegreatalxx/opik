...
You need automated regression testing — but not the kind where you sit down and write a test suite from scratch. The most effective evaluation suites are built incrementally, from real production failures. Every time you find a bad response, you turn it into a test case. Over time, your suite becomes a comprehensive guard against the specific failure modes your agent actually encounters.

<Tip>
  Evaluation suites are created as you debug and improve your agent — they grow organically from real failures, not from a separate test-writing phase.
</Tip>
...
### Add it to an evaluation suite

Turn the failure into a test case. Add the trace to an evaluation suite with a natural-language assertion that captures the expected behavior — for example, *"The response must not hallucinate facts not present in the context"*. You can do this through [Ollie](/tracing/debug-agents) (Opik's AI assistant), the UI, or the SDK.

### Update your agent

Fix the root cause. Update a prompt via [Agent Configuration](/prompt_engineering/getting-started), adjust tool definitions, or change retrieval parameters. Use Ollie to help diagnose the issue and suggest fixes.

### Validate with the evaluation suite

Run the evaluation suite against your updated agent. The suite checks every test case — including the new one — so you confirm the fix works and nothing else regressed.

</Steps>

Each cycle adds a new test case. Over time, your evaluation suite becomes a comprehensive regression guard tailored to the real failure modes of your agent.
...
- [Concepts](/evaluation/concepts) — Understand Evaluation Suites vs Datasets & Metrics
- [Building Evaluation Suites](/evaluation/advanced/building-evaluation-suites) — Create and manage suites via the SDK, UI, or Ollie
