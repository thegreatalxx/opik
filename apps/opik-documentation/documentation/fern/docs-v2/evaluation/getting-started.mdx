...
og:description: Get started evaluating your LLM application with Opik using Evaluation
...
- **Evaluation Suites**: Define assertions in natural language and let an LLM judge test them. Best for pass/fail behavioral testing.
...
<Tabs>
  <Tab title="Evaluation Suites">
    Evaluation Suites let you define expected behaviors as natural-language assertions and run them
    ...
    <CodeBlocks>
      ...
      // Create a suite with assertions
      const suite = await EvaluationSuite.getOrCreate({
          name: "my-agent-tests",
    ...
    Each run creates an experiment in the Opik dashboard for easy comparison. See the
    [Building Evaluation Suites](/evaluation/advanced/building-evaluation-suites) guide for the full walkthrough.
  </Tab>
