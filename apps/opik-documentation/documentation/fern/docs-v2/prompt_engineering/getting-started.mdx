...
Agent Configuration lets you version the prompts, model settings, and parameters that drive your agent — and update them in production without redeploying code. Below you'll set up your first configuration in under five minutes.
...
<Tabs>
  ...
  <Tab title="Manual integration">
    There are two parts to using Agent Configuration: **creating** a configuration version, and **fetching** it at runtime inside your agent.

    ### Step 1 — Define and push your first configuration

    Start by defining the parameters your agent needs and pushing them to Opik. This only needs to happen once (or whenever you want to create a new version programmatically).
    ...
    Inside your agent, call `get_or_create_config` / `getOrCreateConfig` to fetch the active configuration. The `fallback` parameter provides safe defaults your agent can use if Opik is temporarily unreachable.
    ...
    <Warning>
      ...
      (`@opik.track` in Python, `track()` in TypeScript). This is required so the SDK can
      automatically link the configuration version to the trace.
    </Warning>

    <Tip>
      When no `env` or `version` parameter is provided, the SDK defaults to fetching the version
      labeled `prod`. See [Managing versions](#managing-agent-configuration-versions) below to learn
      how to control which version your agent receives.
    </Tip>
  </Tab>
</Tabs>
...
One of the key advantages of Agent Configuration is that you can update your agent's behavior through the Opik platform and deploy changes without triggering a new code release.

This works through **environment labels** and **version pinning**. By default the SDK fetches the version labeled `prod`, but you can control which version is fetched using two parameters:
...
To fetch a specific version or environment, pass the parameter when calling `get_or_create_config` / `getOrCreateConfig`:
